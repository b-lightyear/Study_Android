...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    TextView tv_text ; //findViewById(R.id.tv_text);로도 쓸 수 있지만 setContentView가 이루어지고나서 그 다음에 사용할 수 있음(디자인 연결 후 찾기 )
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rdo_group.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                ...
                //캐스팅에 해당하는 클래스 타입이 어떤 타입인지를 정확히 명시하고 해당하는 클래스의 기능을 사용함.
    ...
    // <=== NG ===> 앱이 종료됨.(개발자가 오류가 날것같은코드를 인지하고 해당하는 부분에 오류가 발생 시 예외처리를 할수가 있다)
    // numberFormatException
    // "AB0" → ?? 잘못된입력
    // " 0" → trim x. 알림 : 공백이나 특수문자 문자열은 사용이 불가능합니다.
    // "-10" → 알림 : 0 이상의 숫자를 입력해주세요.
    public boolean rtnInt(String inputData){ // => int형태로 바뀌고 그리고나서 0이상이면 OK를 리턴하는 메소드
        try{
            if(Integer.parseInt(inputData) >= 0 ) return true; //바뀌는 게 오류가 안나고 0이상이면 true가 리턴됨
        }catch (Exception e){
            e.getStackTrace();
        }
        return false;
